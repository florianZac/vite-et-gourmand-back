...
class MailerService
{
     ...
     * Envoyé automatiquement quand la commande passe au statut "En attente du retour matériel"
     * @param Utilisateur $utilisateur Le client
    ...
    /**
     * @description Envoie un email de pénalité au client lorsque le matériel n'a pas été rendu dans les 10 jours ouvrés
     * Envoyé automatiquement par le cron job app:check-retour-materiel
     * Utilise un template dédié distinct du mail de rappel de retour
     *
    ...
    ): void
    {
        ...
        $html = $this->twig->render('emails/penalite_materiel.html.twig', [
            'prenom'           => $utilisateur->getPrenom(),
            'numero_commande'  => $commande->getNumeroCommande(),
            'date_prestation'  => $commande->getDatePrestation()->format('d/m/Y'),
            'montant'          => $montant,
        ...
        $email = (new Email())  // TemplatedEmail permet l'utilisation des templates Twig
            ...
            ->subject('Pénalité de retard - Matériel non rendu : Commande ' . $commande->getNumeroCommande()) // Sujet dynamique
